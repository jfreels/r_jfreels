...
jf.blend<-function(names,DT=z,values) {
  dat<-dt.dates.filter(DT[variable %in% names])
  dat<-dt.dcast(dat)
  dat<-data.table(date=dat$date,blend=apply(dat[,-1]*values,1,sum))
  data.table(melt(dat,id.vars='date'))
}
